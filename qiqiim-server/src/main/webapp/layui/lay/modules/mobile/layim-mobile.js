...
layui.define(['laytpl', 'upload-mobile', 'layer-mobile', 'zepto'], function (exports) {
    ...
    LAYIM.prototype.setFriendStatus = function (id, type) {
        ...
        console.log("-设置好友在线离线状态，默认就一个好友，固定为在线状态")
        list['removeClass']('layim-list-gray');
    ...
    var comTpl = function (tpl, anim, back) {
        return ['<div class="layim-panel' + (anim ? ' layui-m-anim-left' : '') + '">'
            ...
            , '{{ d.title || d.base.title }}<span class="layim-chat-status"></span> <i class="layui-icon layui-icon-more-vertical" id="layui-icon-close" style="color: white;font-size: 25px;float: right" layim-event="info">&#xe671;</i><i class="layui-icon " style="color: white;font-size: 25px;float: right;margin-right: 20px;" layim-event="reload">&#xe669;</i> '
            , '{{# if(d.data){ }}'
    ...
    var elemTpl = ['<div class="layui-layim">'
        ...
        , '</div>'
        , '<div class="layim-tab-content">'
        , '<ul class="layim-list-top"><li class="layim-null">敬请期待</li>'
    ...
    var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}">'
        ...
        , '<div class="layim-chat-send"><input type="text" id="sendtext" autocomplete="off"><button class="layim-send layui-disabled" layim-event="send">发送</button></div>'
    ...
    // {{ d.mine ? <img src="{{ d.avatar }}" : <img src="layui/images/1.jpg?1599558326279" }}
    var elemChatMain = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">'
        , '<div class="layim-chat-user">{{ d.username!="业务管理部" ? "<img src=\'layui/images/0.jpg?1599558326279\' >" : "<img src=\'layui/images/1.jpg?1599558326279\' >" }}<cite>'
        , '{{ d.username||"佚名" }}'
        , '</cite></div>'
    ...
    var cache = {message: {}, chat: []}, init = function (options) {
        ...
        console.log("-根据提问类型弹出不同的聊天窗口")

        if (mine.sign.search("承保核保") != -1) {
            popchat(yg_hb, true);
        } else if (mine.sign.search("理赔处理") != -1) {
            popchat(yg_hp, true);
        } else {
            $("#layui-icon-close").remove();
        }
        console.log("-启用加载层")
        loading = layer.open({
            type: 2
    ...
    var layimChat, layimMin, To = {}, popchat = function (data, anim, back) {
        ...
        return popchat.index = popPanel({
            ...
            , success: function (elem) {
                ...
                console.log("-检测是否为管理员账号：")
                console.log(cache.mine)
                if (cache.mine.username == "业务管理部") {
                    $("#layui-icon-close").remove();
                }
    ...
    var pushChatlog = function (message) {
        console.log("-存储最近MAX_ITEM条聊天记录到本地")
    ...
    var viewChatlog = function () {
        ...
        console.log("-渲染本地或数据库聊天记录到相应面板");


        var local = layui.data('layim-mobile')[cache.mine.id] || {};
        var thatChat = thisChat(), chatlog = local.chatlog || {},
            thatChatlog = chatlog[thatChat.data.type + thatChat.data.id] || {};
        var ul = thatChat.elem.find('.layim-chat-main ul');

        var thatChatloglength = thatChatlog.length || 0;

        console.log("-本地历史记录为空时，从数据库获取历史记录 thatChatloglength=" + thatChatloglength);
        ...
        if (thatChatloglength == 0) {


            console.log("-获取数据库所有历史记录")
            var data;
            if (mine.sign.search("承保核保") != -1) {
                data = yg_hb;
            } else {
            ...
            $.post('historymessageajax', {id: data.id, pageSize: 2000}, function (str) {
                ...
                result = json.data;
                console.log(result);
                console.log("-数据库历史记录长度：" + result.length);
                if (result.length < 1) {
                    console.log("-历史会话为空时发起提问类型");
                    $("#sendtext").val("咨询类型 ：" + cache.mine.sign)
                    sendMessage();
                }
                // layui.each(result, function (index, item) {
                //     if (new Date().getTime() > item.timestamp && item.timestamp - (sendMessage.time || 0) > 60 * 1000) {
        ...
        console.log("-获取本地所有历史记录")
        // console.log(thatChatlog)
        layui.each(thatChatlog, function (index, item) {
            if (new Date().getTime() > item.timestamp && item.timestamp - (sendMessage.time || 0) > 60 * 1000) {
                ul.append('<li class="layim-chat-system"><span>' + layui.data.date(item.timestamp) + '</span></li>');
                sendMessage.time = item.timestamp;
            }
    ...
    var chatListMore = function () {
        ...
        console.log("-判断展示记录数是否大于限制数" + li.length + ">=" + MAX_ITEM)
        if (li.length >= 0) {
            var first = li.eq(0);//首条记录
            first.prev().remove();
            if (!ul.prev().hasClass('layim-chat-system')) {
                ul.before('<div class="layim-chat-system"><p style="color: #f74c31">咨询类型：' + cache.mine.sign + '</p></div><div class="layim-chat-system"><span layim-event="chatLog">查看更多记录</span></div>');
            }
            // first.remove();
        }
        console.log("-滚动历史记录至最新")
        setTimeout(function () {
            chatMain.scrollTop(chatMain[0].scrollHeight + 200);
        }, 150)
        $(".layui-layim-photos").off("click");
        $(".layui-layim-photos").on('click', function () {
            console.log("-查看大图")
            var src = this.src;
            layer.open({
                content: "<img style='max-width: 100%' src='" + src + "'>"
                , style: 'background-color:#eee;' //自定风格
                , btn: ['返回']
            });
        })
    ...
    var anim = 'layui-anim-upbit', events = {
        ...
        , back: function (othis) {
            ...
            if (index != 1) {
                ...
                window.location.href = "choose.html?userid=" + currentuseraccount + "&username=" + currentusername + "&time=" + new Date().getTime();
            }
        }
        ...
        , info: function (othis) {
            ...
            //询问框
            layer.open({
                content: '<p style="padding-bottom: 8px;text-align: center">咨询完成后请关闭当前聊天<br>出现新问题可重新发起咨询</p>'
                ...
                , shadeClose: false
                , btn: ['暂不关闭', '<span style="color: red">关闭聊天</span>']
                ...
                , no: function (index, layero) {
                    ...
                    layer.open({
                        content: '<p style="padding-bottom: 5px;text-align: center;">确认关闭当前聊天？</p><p style="text-align: center;padding-bottom: 5px;color: red">(此操作不可撤回)</p>'
                        , className: 'layim-about'
                        , shadeClose: false
                        , btn: ['确认关闭', '取消']
                        , yes: function (index, layero) {
                            console.log("-关闭当前聊天，设置本账号失效");
                            //发送消息表示该用户已失效
                            $("#sendtext").val("本次咨询已完成！")
                            sendMessage();
