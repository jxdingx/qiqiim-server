...
layui.define(['layer', 'laytpl', 'upload'], function (exports) {
    ...
    var SHOW = 'layui-show', THIS = 'layim-this', MAX_ITEM = 20;//20条消息记录
    ...
    //设置好友/离线状态
    LAYIM.prototype.setFriendStatus = function (id, type, disablestate) {
        var list = $('.layim-friend' + id);
        console.log("-设置好友在线离线状态,是否已关闭：" + disablestate)
        list[type === 'online' ? 'removeClass' : 'addClass']('layim-list-gray');
        $("#layim-chat-status-type-" + id).html(type === 'online' ? "<span style='color: green'>(在线)</span>" : "<span style='color: grey'>(离线)</span>");
        // invalid
        if (disablestate === '1') {
            $("#layim-chat-status-type-" + id).html("<span style='color: grey'>(已关闭)</span>");
    ...
    var elemTpl = ['<div class="layui-layim-main">'
        ...
        , '<span class="layui-icon layim-status-online" layim-event="status" lay-type="show">&#xe617;在线</span>'
        , '{{# } else if(d.mine.status === "hide") { }}'
        , '<span class="layui-icon layim-status-hide" layim-event="status" lay-type="show">&#xe60f;</span>'
        , '{{# } }}'
        , '<ul class="layui-anim layim-menu-box">'
        , '<li {{d.mine.status === "online" ? "class=layim-this" : ""}} layim-event="status" lay-type="online"><i class="layui-icon">&#xe605;</i>在线</li>'
        ...
        , '</ul>'
        , '</div>'
        , '<input class="layui-layim-remark" placeholder="编辑签名" readonly="readonly" value="{{ d.mine.remark||d.mine.sign||"" }}">'
        , '</div>'
        ...
        // ,' layim-this'
        // ,'{{# } }}'
        // ,'" title="群组" layim-event="tab" lay-type="group1">&#xe613;</li>'
        ...
        , ''
        , '{{# } }}'
        , '" title="代理人" layim-event="tab" lay-type="friend">&#xe612;</li>'
        ...
        , '<li class="layui-icon layim-tool-search" layim-event="search" title="搜索代理人">&#xe615;</li><li class="layui-icon layim-tool-about" layim-event="about" title="关于"></li>'
        , '{{# if(d.base.msgbox){ }}'
        ...
        , '{{# if(!d.base.copyright){ }}'
        , '<li class="layui-icon layim-tool-about" layim-event="about" title="使用说明">&#xe60b;</li>'
        , '{{# } }}'
        , '</ul>'
    ...
    var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
        ...
        , '<span id="layim-chat-status-type-{{ d.data.id }}" ></span><p class="layim-chat-question-type" style="margin-top: 6px;" ><span id="layim-chat-question-type-{{ d.data.id }}" style="color:#ff0000"></span></p>'
        , '</div>'
        , '</div>'
        ...
        , '<span class="layui-icon layim-tool-audio" title="发送常用语"  style="font-size: 16px;" layim-event="media" data-type="common">'

        , '<div class="layim-chat-send" style="position: relative;">'

        , '<span class="layim-send-btn"   style="margin: 0;margin-left: 10px;">常用语</span>'
        , '<span class="layim-send-set" layim-event="setCommon" lay-type="show"><em class="layui-edge"></em></span>'
        , '<ul class="layui-anim layim-menu-box" style="right: auto;top: 33px; width: max-content;">'
        , '<li  layim-event="setCommon" lay-type="很高兴为您服务！"><i class="layui-icon"></i>1、很高兴为您服务!</li>'
        , '<li  layim-event="setCommon"  lay-type="请稍等，为您咨询总公司"><i class="layui-icon"></i>2、请稍等，正在为您咨询总公司</li>'
        , '<li  layim-event="setCommon"  lay-type="请您提供下完整病例资料及客户的体检记录，我们请专人先为您评估"><i class="layui-icon"></i>3、请您提供下完整病例资料及客户的体检记录，我们请专人先为您评估</li>'
        , '</ul>'
        , '</div></span>'
    ...
    var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>'
        , '<div class="layim-chat-user">{{ d.mine ? "<img src=\'layui/images/1.jpg?1599558326279\' >" : "<img src=\'layui/images/0.jpg?1599558326279\' >" }}<cite>'
        , '{{# if(d.mine){ }}'
    ...
    var elemChatList = '<li name="{{ d.data.type }}{{ d.data.id }}" class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="{{ d.data.avatar }}"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
    ...
    var cache = {message: {}, chat: []}, init = function (options) {
        ...
        mine = init.mine || {}
            ...
            console.log("-初始化页面并弹出")
            console.log("-所有代理人数据")
            console.log(data.friend)
            popim(laytpl(elemTpl).render(obj), local);
            ...
            console.log("-设置已上线的会话和联系人为在线状态")
            if (data.friend.length >= 1) {
                $.each(data.friend[0].list, function (index, f) {
                    if (f.status === "online") {
                        layui.layim.setFriendStatus(f.id, f.status, f.disablestate)
                    } else {
                        layui.layim.setFriendStatus(f.id, f.status, f.disablestate)
                    }
                });
    ...
    //显示主面板
    var layimMain, popim = function (content, local) {
        ...
        return layer.open({
            ...
            , success: function (layero) {
                ...
                console.log("-根据历史会话存储生成历史记录")
                if (arr.length > 0) {
                    arr.reverse();
                    historyElem.html(arr.join(''));
                }

                console.log("-生成历史记录判断是否有未读消息并添加红点")
                layui.each(local.history, function (index, item) {
                    // console.log(item)
                    if (item.newstatus == "true") {
                        //新历史会话，生成红点
                        $("#" + item.type + item.id + " .layim-msg-status").css('display', 'block');
                    }
                });
                console.log("-屏蔽右键菜单并自定义")
                banRightMenu();
                // events.sign();
            }
    ...
    var banRightMenu = function () {
        ...
        layimMain.find('.layim-list-history').on('contextmenu', 'li', function (e) {
            var othis = $(this);
            var html = '<ul data-id="' + othis[0].id + '" data-index="' + othis.data('index') + '"><li layim-event="menuHistory" data-type="one">移除该会话</li></ul>';//<li layim-event="menuHistory" data-type="all">清空全部会话列表</li>
    ...
    var layimChat, layimMin, chatIndex, To = {}, popchat = function (data) {
        data = data || {};
        console.log("-点击历史会话或消息提醒打开聊天主界面")
        ...
        console.log("-判断是否有两个及以上的窗口" + chat[0])
        //判断是否有两个及以上的窗口
        if (chat[0]) {
            ...
            if (list.find('li').length === 1 && !listThat[0]) {
                hasFull || layimChat.css('width', 900);
                list.css({
            ...
            hotkeySend();
            $("#layim-chat-question-type-" + data.id).text(data.sign);
        ...
        var index = chatIndex = layer.open({
            ...
            success: function (layero) {
                ...
                console.log("-消除红点，更新历史会话存储");
                console.log(data.type + data.id)
                $("#layim-chat-question-type-" + data.id).text(data.sign);

                $("#" + data.type + data.id + " .layim-msg-status").css('display', 'none');
        ...
        var offsetLeft = $(".layui-layim-chat")[0].offsetLeft;
        var offsetTop = $(".layui-layim-chat")[0].offsetTop;

        $(".layui-layim-chat").css('left', offsetLeft - 150);
        $(".layui-layim-chat").css('top', offsetTop - 10);
    ...
    var syncGray = function (data) {
        ...
        $('.layim-' + data.type + data.id).each(function () {
            if ($(this).hasClass('layim-list-gray')) {
                layui.layim.setFriendStatus(data.id, 'offline', data.disablestate);
            }
        });
    };
    ...
    var changeChat = function (elem, del) {
        ...
        console.log("-切换聊天窗口并取消红点、更新历史会话存储")
        $("#" + elem.attr("name") + " .layim-msg-status").css('display', 'none');
        var local = layui.data('layim')[cache.mine.id] || {}
        var history = local.history;
        var thischatdata = history[elem.attr("name")];
        thischatdata.newstatus = "false";
        history[elem.attr("name")] = thischatdata;
        local.history = history;
        layui.data('layim', {
            key: cache.mine.id
            , value: local
    ...
    var setSkin = function (layero) {
        ...
        var local = layui.data('layim')[cache.mine.id] || {}
            , skin = local.skin;
        skin = "layui/css/modules/layim/skin/2.jpg"
        layero.css({
    ...
    var setHistory = function (data) {
        ...
        if (data.newstatus == "true") {
            //新历史会话，生成红点
            $("#" + data.type + data.id + " .layim-msg-status").css('display', 'block');
        }
    ...
    var messageNew = {}, getMessage = function (data) {
        ...
        console.log("-初始消息处理发送中...")

        if (!data.username) {

            //新增代理人发送消息，先添加好友并刷新
            console.log("-获取新代理人信息");


            $.ajax({
                type: "post",
                url: "./userinfo/queryByUid/" + data.id,
                data: data.field,
                dataType: "json",
                async: false,
                success: function (datadlr) {
                    var dlr = datadlr.data;
                    if (dlr.id) {
                        data.avatar = "layui/images/0.jpg?" + new Date().getTime();
                        data.id = dlr.uid;
                        data.username = dlr.name;
                        data.sign = dlr.signature;
                        data.disablestate = dlr.disablestate;
                        console.log(dlr)

                        addList({
                            type: 'friend' //列表类型，只支持friend和group两种
                            , avatar: "layui/images/0.jpg" //好友头像
                            , username: dlr.name //好友昵称
                            , groupid: 1 //所在的分组id
                            , id: dlr.uid //好友id
                            , sign: dlr.signature ? dlr.signature : '未知问题' //好友签名
                            , disablestate: dlr.disablestate
                        });

                        getMessage_finall(data)
                        return;
    ...
    getMessage_finall = function (data) {
        ...
        if (thatChat.data.type + thatChat.data.id !== data.type + data.id) {
            console.log("-接受到的消息不在当前Tab,显示红点")
            $("#" + data.type + data.id + " .layim-msg-status").css('display', 'block');
    ...
    //接受新代理人问题，添加好友并刷新
    var addList_new = function (data) {

        console.log("--添加代理人")
        var obj = {}, has, listElem = layimMain.find('.layim-list-' + data.type);
        console.log(cache[data.type])
        if (cache[data.type]) {
            if (data.type === 'friend') {
    ...
    var anim = 'layui-anim-upbit', events = {
        ...
        , about: function () {
            ...
            var str = '<div style="margin: 15px;"> <blockquote class="layui-elem-quote"><br> 1、PC端登陆后会实时监听代理人询问情况，请注意浏览器消息提醒！ <br><br> 2、PC端可发送文字/表情/图片/文件/常用语，聊天记录可查看所有记录！ <br><br> 3、代理人发起咨询后，可在<span style="color: red">代理人列表</span>或<span style="color: red">历史会话列表</span>查询，咨询发起后的两个工作日后将会删除，每次咨询请尽量在当日解决！ <br><br> 4、是否在线：蓝色头像为在线  灰色头像为离线<br><br> </blockquote> </div>';
            layer.open({
                type: 1,
                content: str,
                title: '空中销服使用说明'
        ...
        , chat: function (othis) {
            ...
            // $("#" + othis.data('index') + " .layim-msg-status").css('display', 'none');
            var local = layui.data('layim')[cache.mine.id] || {};
                        ...
                        post(members, function (res) {
                            ...
                            ul.find('li').on('click', function () {
                                ...
                                popchat({
                                    ...
                                    , avatar: info.avatar
                                    , id: info.id
                                    , disablestate: info.disablestate
                                });
                                hide();
                            });
        ...
        //常用语快捷键
        , setCommon: function (othis, e) {
            var box = events.setCommon.box = othis.siblings('.layim-menu-box')
                , type = othis.attr('lay-type');

            console.log("显示选项")
            if (type === 'show') {
                if (othis.next().css("display") === 'block') {
                    console.log("隐藏选项")
                    othis.next().css("display", "none")
                    console.log(othis.next().css("display"))
                    return;
                }
                ...
                stope(e);
                box.show().addClass(anim);
                $(document).off('click', events.setCommonHide).on('click', events.setCommonHide);
            } else {
                ...
                var common_thatChat = thisChat()
                common_thatChat.textarea.val(type);
                sendMessage();

                events.setCommonHide(e, othis.parent());
            }
        }, setCommonHide: function (e, box) {
            (box || events.setCommon.box).hide().removeClass(anim);
        }
        ...
        , chatLog: function (othis) {
            console.log("-打开更多记录，页面historymessage")
            var thatChat = thisChat();
        ...
        , menuHistory: function (othis, e) {
            ...
            var none = '<li class="layim-null">当前暂无会话</li>'
