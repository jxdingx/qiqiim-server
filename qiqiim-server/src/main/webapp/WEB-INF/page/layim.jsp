...
<body style="background: url(layui/css/modules/layim/skin/bg.jpg) no-repeat 100%;background-size: cover;">

<%--<div style="margin: 15px;">--%>
<%--    <blockquote class="layui-elem-quote"><br>--%>
<%--        1、PC端登陆后会实时监听代理人询问情况，请注意浏览器消息提醒！--%>
<%--        <br><br>--%>
<%--        2、PC端可发送文字/表情/图片/文件/常用语，聊天记录可查看最近20条记录！--%>
<%--        <br><br>--%>
<%--        3、代理人发起咨询后，可在<span style="color: red">代理人列表</span>或<span style="color: red">历史会话列表</span>查询，咨询发起后的两个工作日后将会删除，每次咨询请尽量在当日解决！--%>
<%--        <br><br>--%>
        ...
        showmsg = function (data) {
            ...
            if (msg.cmd == 3) {
                if (msg.sender != currentsession) {
                    // layer.msg(username+"上线了1");
                    lm.setFriendStatus(msg.getSender(), 'online');
                }
            } else if (msg.cmd == 4) {
                if (msg.sender != currentsession) {
                    // layer.msg(username+"下线了1");
            ...
            var initEventHandle = function () {
                ...
                socket.onmessage = function (event) {
                    if (event.data instanceof ArrayBuffer) {
                        ...
                        var cache = layui.layim.cache();
                        var local = layui.data('layim')[cache.mine.id];
                        var username = "", avatar = "", sign = "", friend = false;
                        layui.each(cache.friend, function (index1, item1) {
                            layui.each(item1.list, function (index, item) {
